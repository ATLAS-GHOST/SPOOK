...
if __name__ == "__main__":
    ...
    parser.add_argument("--target-ip", default="127.0.0.1", help="Local network target IP address ex. 192.168.1.x")
    parser.add_argument("--buffer", type=int, default=2_147_483_647, help="Sender OS buffer size in bytes")
    parser.add_argument("--duration", type=int, default=60, help="How long to send the packets for in seconds")
    parser.add_argument("--packet-size", type=int, default=1400, help="Packet size in bytes")

    args = parser.parse_args()

    if args.packet_size < 16:
        parser.error("--packet-size must be at least 16 bytes (header size is 16).")
    if args.packet_size > 65504:
        parser.error("--packet-size must be at most 65504 bytes.")
    if args.duration < 0:
        parser.error("--duration must be at least greater than 0 seconds.")
    if args.buffer < 0:
        parser.error("--buffer must be at least greater than 0 bytes.")
    
        


    print(f"Sending to destination: {args.target_ip}")
